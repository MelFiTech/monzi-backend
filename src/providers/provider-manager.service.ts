...
import { IWalletProvider, WalletProvider } from './base/wallet-provider.interface';
...
export class ProviderManagerService {
  ...
  async getActiveWalletProvider(): Promise<IWalletProvider> {
    try {
      ...
      if (this.adminConfiguredProvider) {
        ...
        if (provider) {
          this.logger.log(`Using admin-configured provider: ${this.adminConfiguredProvider}`);
          return provider;
        }
      }

      // Second priority: Environment variable
      const envProvider = this.configService.get<string>('DEFAULT_WALLET_PROVIDER', 'SMEPLUG') as WalletProvider;
      const provider = this.providers.get(envProvider);
      
      if (provider) {
        this.logger.log(`Using environment-configured provider: ${envProvider}`);
        return provider;
      }

      // Ultimate fallback to SME Plug
      this.logger.warn('No valid provider configuration found, falling back to SME Plug');
      return this.providers.get(WalletProvider.SMEPLUG);

    } catch (error) {
      this.logger.error('Error getting active provider, falling back to SME Plug:', error);
      return this.providers.get(WalletProvider.SMEPLUG);
    }
  }
   ...
   */
  async switchWalletProvider(newProvider: WalletProvider): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      this.logger.log(`Successfully switched wallet provider to: ${newProvider} (admin override)`);
  ...
  async clearAdminOverride(): Promise<{ success: boolean; message: string }> {
    this.logger.log('Clearing admin provider override, reverting to environment configuration');
    
    this.adminConfiguredProvider = null;
    
    const envProvider = this.configService.get<string>('DEFAULT_WALLET_PROVIDER', 'SMEPLUG');
   ...
   */
  async getAvailableProviders(): Promise<Array<{ name: string; provider: WalletProvider; isActive: boolean }>> {
   ...
   */
  async getWalletProvider(providerName: WalletProvider): Promise<IWalletProvider> {
    const provider = this.providers.get(providerName);
    if (!provider) {
      throw new BadRequestException(`Provider ${providerName} is not available`);
    }
    return provider;
  }
   ...
   */
  async validateProviderConfiguration(provider: WalletProvider): Promise<boolean> {
    try {
