...
export class PolarisWalletProvider implements IWalletProvider {
  ...
  constructor(
    ...
    this.clientConfig = {
      ...
      clientFirstName: this.configService.get<string>(
        'POLARIS_CLIENT_FIRST_NAME',
      ),
      clientLastName: this.configService.get<string>(
        'POLARIS_CLIENT_LAST_NAME',
      ),
      clientMobileNumber: this.configService.get<string>(
        'POLARIS_CLIENT_MOBILE_NUMBER',
      ),
      clientEmails: [
  ...
  async createWallet(data: WalletCreationData): Promise<WalletCreationResult> {
    this.logger.log(
      `Creating Polaris wallet for: ${data.firstName} ${data.lastName}`,
    );
    ...
    try {
      ...
      const payload: PolarisCreateWalletPayload = {
        ...
        transaction: {
          ...
          amount: 0,
          customer: {
            customer_ref: data.phoneNumber
              .replace(/^\+234/, '')
              .replace(/^0/, ''), // Remove +234 and leading 0
      ...
      if (
        ...
        responseData.data.provider_response
      ) {
